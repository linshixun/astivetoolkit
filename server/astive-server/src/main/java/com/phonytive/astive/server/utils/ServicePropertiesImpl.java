...
public class ServicePropertiesImpl implements ServiceProperties {
  ...
  public ServicePropertiesImpl(Properties properties, String serviceName)
    ...
    setDisabled((Boolean.valueOf(properties.get("disabled").toString()).booleanValue()));
    setPort(Integer.valueOf(properties.get("port").toString()).intValue());
    setBacklog(Integer.valueOf(properties.get("threads").toString()).intValue());
    ...
    try {
      setOnlyFrom(InetAddress.getByName(properties.get("onlyFrom").toString()));
    } catch (UnknownHostException ex) {
      throw new SystemException(AppLocale.getI18n("unknownHost",
                                                  new Object[] {
                                                    properties.get("onlyFrom").toString()
                                                  }));
